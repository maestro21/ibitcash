...
function tree($dir_path) {
	...
	print "<tr><td><img src=\"images/ico/folder.png\" width=\"16\" height=\"16\" title=\"Папка\" /></td><td>".$dir_path."</td><td>-</td><td>-</td><td width=\"50\"></td></tr>";
	...
	while ($item = readdir($dir)) {
		if (is_file($path.$item)) {
			...
			if(!mysql_num_rows(mysql_query("SELECT * FROM antivirus WHERE filename = '".$path.$item."'"))) {
				...
				<a href=\"?a=antivirus&act=add&f=".$path.$item."\"><img src=\"images/ico/add.png\" width=\"16\" height=\"16\" alt=\"Нажмите здесь, если уверены в том, что файл чист\" /></a>
				</td></tr>";
			} elseif($time != $fileinfo[9] || $size != filesize($path.$item)) {
				...
				// Если файл чист
				print "<tr><td>".$prefix." <img src=\"images/ico/".whatfile($item).".png\" width=\"16\" height=\"16\" title=\"Файл\" /> </td><td>".$prefix." ".$item."</td><td>".filesize($path.$item)."</td><td>".date("d.m.Y H:i", $fileinfo[9])."</td><td></td></tr>";
			}

		} elseif (is_dir($path.$item)) {
			if ($item != "." && $item != "..") {
				print "<tr><td>".$prefix."<img src=\"images/ico/folder.png\" width=\"16\" height=\"16\" title=\"Папка\" /></td><td> ".$prefix." ".$item."</td><td>-</td><td>-</td><td></td></tr>";
				$new_dir = opendir($path.$item);
