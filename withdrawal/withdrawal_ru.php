...
$m_eth=number_format(mysql_result($sql1,0,'min_eth'),8,'.', '');
	if ($_GET['action'] == 'save') {
		...
		if ($sum <= 0) {
			...
			$sql_us=mysql_query("SELECT * FROM users WHERE login='$login'");
			...
			switch ($ps)
			{
				case 3:
					...
					{
						$sum1=$cur_sum-$sum;
						mysql_query("UPDATE users SET btc = '$sum1' WHERE login = '$login'");
						$k=1;
					}
					break;
				case 4:
					...
					{
						$sum1=$cur_sum-$sum;
						mysql_query("UPDATE users SET dgc = '$sum1' WHERE login = '$login'");		
						$k=1;				
					}
					break;
				case 5:
					...
					{
						$sum1=$cur_sum-$sum;
						mysql_query("UPDATE users SET ltc = '$sum1' WHERE login = '$login'");	
						$k=1;	
				...
				case 6:
					...
					{
						$sum1=$cur_sum-$sum;
						mysql_query("UPDATE users SET dsh = '$sum1' WHERE login = '$login'");	
						$k=1;						
					}
					break;
				case 7:
					...
					{
						$sum1=$cur_sum-$sum;
						mysql_query("UPDATE users SET mnr = '$sum1' WHERE login = '$login'");		
						$k=1;						
					}
					break;
				case 8:
					...
					{
						$sum1=$cur_sum-$sum;
						mysql_query("UPDATE users SET eth = '$sum1' WHERE login = '$login'");	
						$k=1;						
					}
					break;
			}
...
<li class="active_l"><a href="/withdrawal/">Вывести средства</a></li>
					 ...
					 <div class="right_div">
							...
							<div class="pay_content">
								<div class="tab_pane" id="tab2BTC">
										...
										Доступно:<span id='mxbal'><?echo $btc?></span><br>Минимальная сумма:<span id='mbal'><?echo $m_btc?></span></p>
												<div class="forms_right">
												<form action="?action=save" method="post">
												<input type=hidden id="ps" name="ps" value='3'>
													<input type="text"  id="purse" name='purse' class="drawal" placeholder="&nbsp;&nbsp;КОШЕЛЕК ПОЛУЧАТЕЛЯ" required><br>
													<input type="text" autocomplete='off' id="sum" value='<?echo $ibtc;?>' name='sum' class="inp_btc" placeholder='&nbsp;&nbsp;BTC'>
													<button class="exchange_button" type="submit" style='width:16%;margin-left:2%'>Вывод среств</button><br>	 
													
													</form>
													<p>Бонусных DOGE:<?echo $bdgc?></p>
													<p>Бонусных Ethereum:<?echo $beth?></p>
												</div>
								</div>
							</div>
					 </div>
